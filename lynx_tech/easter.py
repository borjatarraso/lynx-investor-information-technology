...
def tk_fireworks(root) -> None:
    ...
    def _animate():
        ...
        particles = alive
        if step[0] == 1:
            for i, msg in enumerate(["\u2605  lynx-tech  \u2605", "Information Technology Analysis", "\U0001f680 To the Moon! \U0001f680"]):
...
def tk_rainbow_title(root, count: int = 20) -> None:
    ...
    def _step(remaining, idx):
        if remaining <= 0:
            ...
            return
        try: root.title(f"{random.choice(sparkles)} {colors[idx % len(colors)]} lynx-tech {colors[idx % len(colors)]} {random.choice(sparkles)}")
        except tk.TclError: return
