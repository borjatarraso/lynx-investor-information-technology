"""IT-specialized metrics calculation engine.

All calculations are both tier-aware AND stage-aware. This module computes
traditional financial metrics plus IT-specific indicators:

  * Rule of 40 (revenue growth % + FCF/EBITDA margin %)
  * Magic Number (ΔARR * 4 / S&M) — sales efficiency
  * R&D Intensity (R&D / revenue)
  * Stock-Based Compensation ratios (SBC / revenue, SBC / FCF)
  * Rule of X (Altimeter) = rev growth × (1 + FCF margin)
  * Revenue per Employee (productivity)
  * Deferred Revenue ratio (forward-visibility proxy for ARR)
  * Goodwill-to-Assets (M&A-heavy tech impairment risk)
  * CAC Payback (approximate, using S&M / gross profit growth)
"""
...
from lynx_tech.models import (
    ...
    TechQualityIndicators, ProfitabilityMetrics, ShareStructure,
...
def calc_growth(
    ...
    # We leave these as None and highlight the data gap in tech-quality scoring.
...
def calc_share_structure(
    ...
    # Tech companies tend to run larger float counts; thresholds differ from other sectors
...
def calc_tech_quality(
    ...
    """Composite quality score 0-100 with IT-specific axes.
    ...
    if gm is not None:
        if gm >= 0.80:
            ...
            m.moat_type = "Scalable SaaS / platform economics"
            score += 20
        elif gm >= 0.70:
            m.moat_assessment = "Strong — 70-80% gross margin"
            m.moat_type = "Classical software moat"
            score += 16
        elif gm >= 0.55:
            m.moat_assessment = "Solid — 55-70% gross margin"
            m.moat_type = "Hybrid SaaS/services or infrastructure"
            score += 12
        elif gm >= 0.35:
            m.moat_assessment = "Average — 35-55% gross margin"
            m.moat_type = "Hardware, services, or low-moat model"
            score += 6
        else:
            m.moat_assessment = "Weak — <35% gross margin"
            m.moat_type = "Commodity hardware or services"
            score += 2
    else:
    ...
    if m.quality_score >= 75:
        m.competitive_position = "Best-in-Class — Elite Tech Franchise"
...
def calc_intrinsic_value(
    ...
    # Method selection for IT sector
...
def calc_market_intelligence(
    ...
    # ── Tech benchmark context — QQQ as headline benchmark + sub-sector ETF ───
    from lynx_tech.models import TechCategory, classify_category
    _CATEGORY_ETFS = {
        TechCategory.SAAS: [("IGV", "iShares Expanded Tech-Software"),
                            ("WCLD", "WisdomTree Cloud Computing")],
        TechCategory.INFRA_SOFTWARE: [("IGV", "iShares Expanded Tech-Software"),
                                      ("XLK", "Tech Select Sector SPDR")],
        TechCategory.CLOUD: [("WCLD", "WisdomTree Cloud Computing"),
                             ("SKYY", "First Trust Cloud Computing")],
        TechCategory.CYBERSECURITY: [("CIBR", "First Trust NASDAQ Cybersecurity"),
                                     ("HACK", "Amplify Cybersecurity ETF")],
        TechCategory.SEMICONDUCTOR: [("SMH", "VanEck Semiconductor"),
                                     ("SOXX", "iShares Semiconductor")],
        TechCategory.SEMI_EQUIPMENT: [("SMH", "VanEck Semiconductor"),
                                      ("SOXX", "iShares Semiconductor")],
        TechCategory.IT_SERVICES: [("XLK", "Tech Select Sector SPDR"),
                                   ("VGT", "Vanguard Information Technology")],
        TechCategory.HARDWARE: [("XLK", "Tech Select Sector SPDR"),
                                ("VGT", "Vanguard Information Technology")],
        TechCategory.INTERNET: [("FDN", "First Trust Dow Jones Internet"),
                                ("XLC", "Communication Services Select")],
        TechCategory.FINTECH: [("FINX", "Global X FinTech ETF"),
                               ("IPAY", "ETFMG Prime Mobile Payments")],
        TechCategory.DATA_AI: [("AIQ", "Global X Artificial Intelligence"),
                               ("BOTZ", "Global X Robotics & AI")],
        TechCategory.OTHER: [("XLK", "Tech Select Sector SPDR"),
                             ("VGT", "Vanguard Information Technology")],
    }
    ...
    # Headline tech benchmark: QQQ (Invesco Nasdaq-100 ETF)
    try:
        qt = yf.Ticker("QQQ")
        qi = qt.info or {}
        mi.benchmark_name = "Nasdaq-100 (QQQ)"
        mi.benchmark_ticker = "QQQ"
    ...
    # Sub-sector ETFs
    try:
        etf_list = _CATEGORY_ETFS.get(category, _CATEGORY_ETFS[TechCategory.OTHER])
        if len(etf_list) >= 1:
    ...
    # ── Risk warnings (IT-specific) ─────────────────────────────────
    warnings: list[str] = []

    if mi.beta and mi.beta > 1.5:
        warnings.append(
            f"Elevated volatility (beta {mi.beta:.1f}) — amplified market swings typical for growth tech"
        )
    ...
    if stage in growth_stages and solvency and solvency.total_debt and solvency.total_debt > 0:
        if solvency.net_debt and solvency.net_debt > 0:
            warnings.append("Net debt in growth-stage tech — watch refinancing risk")

    mi.risk_warnings = warnings

    # ── IT disclaimers ──────────────────────────────────────────────
    disclaimers: list[str] = [
        "Tech investments exhibit rapid valuation shifts tied to growth & rate cycles.",
        "SaaS/platform models rely on retention and renewal — churn can break the thesis.",
        "Stock-based compensation (SBC) creates real shareholder dilution even when non-cash.",
    ]
    if stage == CompanyStage.STARTUP:
        disclaimers.append(
            "Early-stage tech: product-market fit, not financials, dominates risk/reward."
        )
    if stage == CompanyStage.GROWTH:
        disclaimers.append(
            "Hyper-growth multiples compress dramatically when growth decelerates — monitor NRR and cohorts."
        )
    if category.value.startswith("Semiconductor"):
        disclaimers.append(
            "Semiconductors are highly cyclical with 3-5 year design/capacity cycles."
        )
    disclaimers.extend([
...
calc_mining_quality = calc_tech_quality  # pragma: no cover
