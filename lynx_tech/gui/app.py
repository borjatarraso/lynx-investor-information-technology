...
class SplashScreen:
    ...
    def __init__(self, root: tk.Tk, on_done: callable) -> None:
        ...
        # Version
        from lynx_tech import __version__, __year__, __author__, SUITE_LABEL
...
class LynxTechGUI:
    ...
    def _after_splash(self) -> None:
        ...
        from lynx_tech.tui.themes import HOUSE_THEMES as _HOUSE_THEMES
        from lynx_investor_core.gui_themes import register_gui_themes
        register_gui_themes(*_HOUSE_THEMES)
        self._theme_cycler = ThemeCycler(self.root, start="tech-dark")
    ...
    def _show_themes_menu(self) -> None:
        ...
        sector_family = ["tech-dark", "tech-light", "cybrdots"]
    ...
    def _show_controls(self) -> None:
        ...
        self._show_info_popup(
            ...
            "lynx-tech -- Graphical Interface",
            sections,
        )

    def _on_about(self) -> None:
        from lynx_tech import get_about_text
        about = get_about_text()

        win = tk.Toplevel(self.root)
        win.title("About lynx-tech")
        win.configure(bg=BG)
    ...
    def _ee_shake(self) -> None:
        from lynx_tech.easter import tk_fireworks
        tk_fireworks(self.root)

    def _ee_rainbow(self) -> None:
        from lynx_tech.easter import tk_rainbow_title
        tk_rainbow_title(self.root)

    def _ee_fortune(self) -> None:
        from lynx_tech.easter import FORTUNE_QUOTES
        import random
    ...
    def _run_analysis(self, identifier: str) -> None:
        try:
            ...
            from lynx_tech.core.storage import is_testing
    ...
    def _render_stage(self, stage: str, report: AnalysisReport) -> None:
        ...
        try:
            ...
            elif stage == "tech_quality":
                self._render_tech_quality(report)
    ...
    def _render_profile(self, r: AnalysisReport) -> None:
        ...
        category_text = p.tech_category.value if hasattr(p.tech_category, 'value') else str(p.tech_category)
        ...
        rows: list[tuple[str, str, str]] = [
            ...
            ("Stage", stage_text, FG),
            ("Tech Category", category_text, YELLOW),
        ...
        if all_irrelevant:
            ...
            note = tk.Label(
                ...
                text=(f"Profitability metrics are not applicable for {stage_name} stage companies. "
                      f"Pre-revenue tech startups have no meaningful margins or returns on capital to evaluate."),
    ...
    def _render_tech_quality(self, r: AnalysisReport) -> None:
        m = r.tech_quality
        if m is None:
            return
        card = CollapsibleCard(
            self.scroll_frame, "Tech Quality Indicators",
            icon=ICON_TECH, accent=YELLOW, expanded=False,
            info_command=lambda: self._show_section_info("tech_quality"),
        )
        self._sections.append(card)
        frame = card.frame
    ...
    def _render_screening(self, r: AnalysisReport) -> None:
        """Render the 10-point tech screening checklist."""
        ...
        card = CollapsibleCard(
            self.scroll_frame, "Tech Screening Checklist",
    ...
    def _render_conclusion(self, r: AnalysisReport) -> None:
        ...
        # Category scores (using tech_quality instead of moat)
        sep = tk.Frame(frame, bg=BORDER, height=1)
        sep.pack(fill=tk.X, padx=12, pady=6)
        for i, cat in enumerate(("valuation", "profitability", "solvency", "growth", "tech_quality")):
    ...
    def _show_section_info(self, section_key: str) -> None:
        from lynx_tech.metrics.explanations import get_section_explanation
    ...
    def _show_conclusion_info(self, category: str = "overall") -> None:
        from lynx_tech.metrics.explanations import get_conclusion_explanation
