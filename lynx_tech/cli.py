"""Command-line interface for lynx-tech."""
...
from lynx_tech import __author__, __author_email__, __license__, __version__, __year__, SUITE_LABEL
...
def build_parser():
    parser = argparse.ArgumentParser(
        prog="lynx-tech",
        description="Lynx Information Technology Analysis — SaaS, Software, Cloud, Cybersecurity, Semiconductors & IT fundamental analysis tool.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog=(
            "Examples:\n"
            "  lynx-tech -p MSFT                            Mega-cap platform\n"
            "  lynx-tech -p NVDA                            Semiconductor leader\n"
            "  lynx-tech -p CRM                             Enterprise SaaS\n"
            "  lynx-tech -p CRWD --refresh                  Force fresh data\n"
            '  lynx-tech -p "Palantir"                      Search by name\n'
            "  lynx-tech -p -i                              Interactive mode\n"
            "  lynx-tech -t MSFT                            Testing mode\n"
        ),
    )
...
def run_cli():
    ...
    if "--b2m" in sys.argv:
        ...
        from lynx_tech.easter import rich_rocket, rich_fortune
        c = Console()
        rich_rocket(c)
        rich_fortune(c)
        return
    ...
    if "--about" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from lynx_tech import get_about_text
    ...
    if "--explain" in sys.argv:
        from rich.console import Console; from rich.panel import Panel; from rich.table import Table
        from lynx_tech.metrics.explanations import get_explanation, list_metrics
    ...
    from lynx_tech.core.storage import set_mode, is_testing
    ...
    if args.interactive:
        from lynx_tech.interactive import run_interactive; run_interactive(); return
    if args.tui:
        from lynx_tech.tui.app import run_tui; run_tui(); return
    if args.gui:
        from lynx_tech.gui.app import run_gui; run_gui(args); return
    if args.search:
    ...
    from lynx_tech.display import display_report_stage
    try:
        ...
        if args.export:
            from pathlib import Path; from lynx_tech.export import ExportFormat, export_report
...
def _cmd_list_cache(con):
    from rich.table import Table; from lynx_tech.core.storage import list_cached_tickers, get_mode
